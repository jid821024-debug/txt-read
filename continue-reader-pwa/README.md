...
PC, 모바일, 태블릿에서 같은 계정으로 로그인하여 TXT 문서를 읽고, 마지막 읽은 위치부터 이어서 보거나 기기 내장 음성으로 들을 수 있는 개인용 PWA입니다.
...
- 기기에 설치된 한국어 목소리 선택
- 읽는 문단 자동 강조 및 화면 이동
- 음성으로 읽은 글자 위치 자동 저장 및 다른 기기와 동기화
- 잠금화면 미디어 버튼과 백그라운드 재생 보조(Android Chrome/PWA 우선)
...
├─ audio/
│  └─ silence.wav            ← 잠금화면 재생 보조용 무음 오디오
├─ sw.js
├─ vercel.json
├─ icons/
│  ├─ icon-192.png
...
1. 문서를 열고 듣고 싶은 위치로 이동합니다.
2. 화면 아래의 **▶ 읽기**를 누릅니다.
3. 같은 버튼으로 일시정지하고, 다시 누르면 같은 위치부터 계속됩니다.
4. **−15 / +15** 버튼으로 약 100글자씩 앞뒤로 이동할 수 있습니다.
5. 속도 막대에서 0.6배~2.0배로 조절합니다.
6. `Aa` 설정에서 목소리, 자동 이동, 화면 꺼짐 재생 보조를 선택합니다.
7. 읽은 위치는 문단 번호와 글자 위치로 저장되어 일반 이어읽기 위치와 함께 동기화됩니다.

## 화면을 끄고 듣기

- Android에서는 Chrome으로 접속한 뒤 **앱 설치**하여 PWA로 실행하는 방식이 가장 안정적입니다.
- **화면 꺼짐 재생 보조**를 켜면 무음 오디오와 Media Session을 사용해 잠금화면 재생 버튼 연결을 시도합니다.
- 기기 제조사 배터리 절전 설정에서 Chrome 또는 설치한 이어읽기 앱의 백그라운드 실행을 허용하면 더 안정적입니다.
- iPhone·iPad는 Web Speech API가 화면 잠금이나 장시간 백그라운드 상태에서 중단될 수 있습니다. 이는 웹앱 코드만으로 완전히 우회할 수 없는 운영체제 제한입니다.
- 화면이 다시 켜졌을 때 음성이 중단된 상태이면 저장된 글자 위치부터 자동 재시작을 시도합니다.

## 음성 관련 참고

- 음성 품질과 목소리 목록은 Windows, Android, iPhone 등 기기에 설치된 음성 엔진에 따라 달라집니다.
- 한국어 목소리가 목록에 없으면 운영체제의 언어·접근성·TTS 설정에서 한국어 음성을 설치해야 합니다.
- 음성 데이터는 별도 서버로 업로드하지 않으며, 브라우저/운영체제의 내장 음성 기능을 사용합니다.
